...
This allows for proper encapsulation at runtime as all field calls will be replaced by the corresponding getter/setter calls.
...
NOTE: The rest of the documentation shows usages based on the active record pattern only,
...
IDs are often a touchy subject, and not everyone's up for letting them be handled by the framework, once again we
have you covered.
