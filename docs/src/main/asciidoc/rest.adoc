...
This will cause all rest endpoints to be resolved relative to `/api`, so the endpoint above with `@Path("rest")` would
...
|The value of a https://tools.ietf.org/html/rfc3986#section-3.3[URI path segment parameter]
...
This can sometimes make it hard to understand why certain HTTP status codes are returned (as Jakarta REST mandates the use of non-intuitive error codes in various cases).
...
You can group your request parameters in a container class instead of declaring them as method parameters to your endpoint,
...
[TIP]
====
When a Resource method needs to handle various types of multipart requests, then the `org.jboss.resteasy.reactive.server.multipart.MultipartFormDataInput` method type can be used as it provides access to all the parts of the request.
...
Similarly, Quarkus REST can produce Multipart Form data to allow users to download files from the server. For example, we could write a POJO
...
This last approach allows you to add extra headers to the output part.
...
[NOTE]
====
There are situations where various Jackson related exceptions need to be handled in a uniform way. For example, the application may need to handle all `JsonMappingException` the same way.
This becomes a problem when taking JAX-RS / Jakarta REST rules into account, because the exception mapper `ExceptionMapper` for `MismatchedInputException` would be used instead of the user-provided
...
[NOTE]
====
Quarkus will automatically find all the classes annotated with `@XmlRootElement` and then bind them to the JAXB context.
====
...
[NOTE]
====
It's not necessary to implement all three methods, but only the ones you need.
====
...
[NOTE]
====
Exception mappers defined in REST endpoint classes will only be called if the  exception is thrown in the same class. If you want to define global exception mappers,
...
For 3 and 4, constructor instantiation is prioritized above method instantiation. Public, protected, and package private constructors and methods are supported for instantiation.
