...
When running the production version of the application, the Infinispan connection needs to be configured as normal,
...
=== Accessing the Infinispan Server Console
...
However, there are cases where it is helpful to provide some extra configuration to the server.
...
If you want to run the Infinispan Server container with Cross Site Replication configuration, you need to provide a site name.
...
at least one property in the application properties:
...
<2> Enables tracing.
<3> Exporter protocol. OTLP is the OpenTelemetry protocol.
