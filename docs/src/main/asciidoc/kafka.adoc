...
As with the previous `Message` example, if your injected channel receives payloads (`Multi<T>`), it acknowledges the message automatically, and supports multiple subscribers.
...
- `ignore`: the failure is logged, but the processing continues. The offset of the record that has not been processed correctly is committed.
...
However, a consumer can be assigned multiple partitions if the number of partitions is greater than the number of consumers in the group.
...
Note that if the `concurrency` value exceeds the number of partitions of the topic, some consumer threads won't be assigned any partitions.
+
...
+
Lastly different applications can subscribe independently to the same topics using different *consumer group ids*.
...
The default partitioner uses record key hash to compute the partition for a record, or when the key is not defined, chooses a partition randomly per batch of records.
...
To achieve this, implement the `io.smallrye.reactive.messaging.kafka.KafkaConsumerRebalanceListener` interface and expose it as a CDI bean with the `@Identifier` qualifier.
...
IMPORTANT: If the `group.id` attribute is not set, it defaults to the `quarkus.application.name` configuration property.
...
In manual topic-partition assignment, rebalancing doesn't happen and therefore rebalance listeners are never called.
...
Then, the connector writes a new message to Kafka when one of the broker’s in-flight messages gets acknowledged.
...
If the message order is a must for your use case, setting `max.in.flight.requests.per.connection` to `1` will make sure a single batch of messages is sent at a time, at the expense of limiting the throughput of the producer.
...
If you want to deserialize a list of fruits, you need to create a deserializer with a Jackson `TypeReference` denoting the generic collection used.
...
If you want to deserialize a list of fruits, you need to create a deserializer with a `Type` denoting the generic collection used.
...
# Incoming channel (receiving records from Kafka)
...
and for an outgoing channel checks that the topic used by the producer exists in the broker.
...
When disabled, the `AppInfoParser` skips the JMX MBean registration and the default value for `kafka.metric.reporters` is set to empty, preventing the `JmxReporter` from being loaded.
...
OAuth authentication works for both JVM and native modes. Since SSL is not enabled by default in native mode, `quarkus.ssl.native=true` must be added to support JaasClientOauthLoginCallbackHandler, which uses SSL. (See the xref:native-and-ssl.adoc[Using SSL with Native Executables] guide for more details.)
...
If your Kafka consumer is batch based, you will need to send a batch of messages to the channel by creating them manually.
...
<6> Close the session - this closes the connection with the database. The connection can then be recycled.
...
2. You create a Hibernate entity instance from this payload,
...
Some environments cut the SSE connection when there is not enough activity.
...
<7> Once the persist operation completes, and there are no errors, the Kafka transaction is committed.
