...
If you want to run the server yourself using Docker, check out the 5-minute https://infinispan.org/get-started/[Getting started with Infinispan]
...
This extension lets you configure a _default_ Infinispan client connection and _named_ ones.
...
[WARNING]
====
The `configuration-resource` is a build time property and the file will be included in the native build automatically.
...
[TIP]
====
Infinispan caches can store keys and values in different encodings, but we recommend using https://developers.google.com/protocol-buffers[Protocol Buffers (Protobuf)].
...
<3> Indicates the `description` field should be indexed as a Keyword.
...
message Book {
  ...
  required double price = 5; // no native BigDecimal type available in Protobuf but you can use the adapter
}
...
Remove the unnecessary indirection by using the `query` method in the `RemoteCache` API as follows.
...
Near caching is disabled by default, but you can enable it on a per cache basis by configuring the following properties:
...
and configures your application.
