...
In fact, the interesting part is that we will need to add very few elements to make our full text search application work.
...
<3> Create a "search scope" targeting all indexed entity types that extend `Object` -- that is, every single indexed entity type (`Author` and `Book`).
